...
object ProgressHud {
    ...
    fun register() {
        HudRenderCallback.EVENT.register { context, _ ->
            val progress = ImportProgress.current() ?: return@register
            render(context, progress)
        }
    }

    private fun render(context: DrawContext, progress: HouseProgress) {
        val font = TestMod.MC.textRenderer
        ...
        context.drawTextWithShadow(font, heading, X, Y + BAR_HEIGHT + 3, HEADING_COLOR)
        context.drawTextWithShadow(font, detail, X, Y + BAR_HEIGHT + 14, DETAIL_COLOR)
    }
